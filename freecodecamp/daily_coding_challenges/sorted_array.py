...
# More Pythonic
# Created adjacent pairs with zip()
# Checked the conditions across sequences with all()
# Used generator expressions for verbosity
# "Lazy evaluation over intermediate lists"

def is_sorted(arr):
    if all(a > b for a, b in zip(arr, arr[1:])):
        return "Descending"
    elif all(a < b for a, b in zip(arr, arr[1:])):
        return "Ascending"
    else:
        return "Not sorted"